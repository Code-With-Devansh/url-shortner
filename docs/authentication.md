...
> **Fixed bug (see `database.md` Redis key map):** the IP portion of these keys previously came from `ipKeyGenerator(req)` instead of `ipKeyGenerator(req.ip)`, which coerced to the literal string `"[object Object]"` and silently collapsed the IP dimension for `login`, `refresh`, and `api` limiters (and the IP-fallback branch of `email`) in production. This meant login/refresh limiting was effectively keyed by email/deviceId alone, not IP+email/IP+deviceId as documented above, until it was fixed. Dev never caught this because rate limiters are skipped entirely when `NODE_ENV=development`.
