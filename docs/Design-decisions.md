...
## Per-user live Redis aggregates instead of a Mongo URL-list fan-out

Account-wide analytics endpoints (summary, timeseries, leaderboard) originally resolved "today's live data" by first querying MongoDB for every URL id the user owns, then fanning out over per-URL Redis structures keyed by those ids. That's fine for an account with a handful of URLs, but both the Mongo query and the fan-out scale with the account's *total* URL count, not its activity — a user with hundreds of thousands of URLs paid that cost on every request, even if only a handful of those URLs had ever been clicked. The fix replaces the Mongo lookup with two Redis structures maintained incrementally on every click: a per-user string counter (`analytics:live:total:{userId}`) for the O(1) total, and a per-user sorted set (`analytics:live:{userId}`, member = urlId, score = unflushed clicks) that serves the leaderboard directly via `ZREVRANGE` and doubles as the index of "which URLs are active today" for anything that still needs a bounded fan-out (unique-visitor HLL keys, breakdown). The trade-off: every click belonging to an owned URL now does 2 extra Redis writes (an `INCR` and a `ZINCRBY`, both in the same pipeline as the existing minute-bucket write, so no extra round-trip) instead of zero. Given the write-heavy path (every click) was already cheap and the read-heavy path (account-wide analytics) was the thing scaling badly, this was judged the right direction to push the cost — see [`Challenges-and-tradeoffs.md`](./Challenges-and-tradeoffs.md) for the specific bug this fixes and the alternatives considered along the way.

**Why breakdown didn't get the same live-aggregate treatment.** The natural extension would be a live per-dimension hash per user (`analytics:live:country:{userId}`, etc.) so breakdown could also skip the Redis fan-out entirely. This was deliberately not built: it would mean 5+ additional Redis writes on every single click (one per dimension) to speed up an endpoint that's read far less often than summary/timeseries/leaderboard — pushing cost onto the hottest path in the system to save time on one of its coolest. Breakdown still fans out over per-URL minute-bucket hashes, just over a candidate list bounded by today's active URLs (from the live ZSET) instead of the account's total URL count — a smaller, but not zero, fan-out. If breakdown traffic or active-URL-per-day counts ever grow enough to matter, the next lever is a short TTL cache on the breakdown result specifically (mirroring the existing `CACHE_TTL.historical` pattern), not live per-dimension writes on the hot path.
