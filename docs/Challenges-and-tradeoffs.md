...
## Account-wide analytics fanning out over a Mongo URL list

**Problem:** `getOverallAnalyticsSummary`, `getOverallAnalyticsTimeseries`, `getOverallAnalyticsBreakdown`, and `getOverallAnalyticsLeaderboard` all started by calling `getUserUrlIds(userId)` — a `ShortUrl.find({ user: userId }, "_id")` query — and then used that id list to fan out over per-URL Redis structures (`getActiveBucketKeysForUrls`, a per-URL total lookup for the leaderboard). That's cheap for an account with a few URLs, but both the query and the fan-out scale with the account's *total* URL count, not its activity. An account with hundreds of thousands of URLs paid that cost on every account-wide analytics request, regardless of how many of those URLs had ever actually been clicked — the exact "account-size-independent" property the rest of the analytics pipeline was designed around didn't hold for this one code path.

**Resolution:** two per-user Redis structures — a string counter (`analytics:live:total:{userId}`) and a sorted set (`analytics:live:{userId}`, member = urlId, score = unflushed clicks) — are now maintained incrementally on every click and decremented at flush time by exactly the amount just persisted (`decrementLiveCounters`, run after the Mongo transaction commits, safe as plain commands rather than Lua since the bucket is already exclusively claimed by that point). Summary/timeseries reads become a single `GET`; the leaderboard reads a direct `ZREVRANGE ... WITHSCORES` top-K instead of fanning out over every owned URL. `getUserUrlIds` was removed entirely.

**What's still fan-out, and why that's accepted:** the breakdown endpoint (per-dimension counts — country/browser/device/OS/referrer) and the HLL merge for summary/timeseries still need to touch actual per-URL Redis keys, because a scalar counter can't carry dimension breakdowns and HLL sketches aren't additive from a total. Both now source their candidate URL list from the live ZSET's members instead of Mongo, so the fan-out is bounded by **today's active URL count**, not the account's total URL count — smaller, but not eliminated. Going further (live per-dimension hashes, so breakdown could also skip the fan-out) was considered and rejected: it would add several extra Redis writes to every single click — the hottest path in the system — to speed up an endpoint that's read comparatively rarely. The cost was judged not worth paying until there's evidence breakdown itself is slow in practice, at which point a short-TTL cache on that specific endpoint is the next lever, not more live writes.
