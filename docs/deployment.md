...
|---|---|---|
| `*/1 * * * *` (every minute) | `analyticsWorker.js` | Flushes due, fully-elapsed **per-minute** Redis click-dimension buckets (`analytics:{urlId}:{date}:{HH:MM}`) into `ClickBucket` documents in MongoDB via `$inc` — see [`ANALYTICS.md`](./ANALYTICS.md#write-pipeline) and [Minute-Bucketed Buffering](./ANALYTICS.md#minute-bucketed-buffering). |
| `*/1 * * * *` (every minute) | `flushClicksWorker.js` | Drains the Redis `clicks` hash into the denormalized `ShortUrl.clicks` counter — see [`DATABASE.md`](./DATABASE.md#shorturl). |
...
**`analyticsWorker` runs every minute, but doesn't necessarily flush everything it sees on every run.** Redis buckets are scoped per-minute specifically so the flush job never has to read-then-delete a key a concurrent click could still be writing to — `flushAnalyticsKey` skips (no-ops on) any bucket whose 60-second window hasn't fully elapsed plus a further grace period, leaving it for a later run. Combined with `ClickBucket` now being written via `$inc` rather than `$set` (so repeated same-day flushes accumulate instead of clobbering each other), this bounds analytics staleness to roughly 1–2 minutes end-to-end — a meaningful tightening from the previous once-daily schedule, and the durability exposure window discussed in [`ANALYTICS.md`](./ANALYTICS.md#known-gaps--accepted-risk) is correspondingly small, though still not zero.
...
- **No automated database backup/restore process is documented here.** If Atlas's built-in continuous backups are relied on, that should be stated explicitly rather than assumed — see the same note in [`DATABASE.md`](./DATABASE.md#known-gaps--accepted-risk).
...
- **The `analyticsWorker`/`flushClicksWorker` cadence (every minute) still leaves a small, non-zero durability exposure window** for click analytics sitting only in Redis before it's flushed — see [Scheduled Jobs](#scheduled-jobs) and the fuller discussion in [`ANALYTICS.md`](./ANALYTICS.md#known-gaps--accepted-risk).
