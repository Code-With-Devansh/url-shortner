...
|---|---|---|
| `0 3 * * *` (once daily, 3am) | `analyticsWorker.js` | Flushes the Redis per-day click-dimension hashes (`analytics:{urlId}:{date}`) into `ClickBucket` documents in MongoDB — see [`ANALYTICS.md`](./ANALYTICS.md#write-pipeline). |
...
**The two jobs run on very different cadences on purpose** (once a day vs. once a minute), reflecting the different cost/staleness trade-offs of what they flush — but it's worth being deliberate about this: the `analyticsWorker` schedule directly sets the upper bound on how much analytics data a Redis failure could lose before it's durably written (see [`ANALYTICS.md`](./ANALYTICS.md#known-gaps--accepted-risk)). A once-daily cadence means up to a full day of click analytics is only ever "at risk" in Redis at any given time — worth tightening if that risk profile ever needs to change, independent of any other durability work.
...
- **The `analyticsWorker` cron cadence (once daily) sets a wide durability exposure window** for click analytics sitting only in Redis — see [Scheduled Jobs](#scheduled-jobs) and the fuller discussion in [`ANALYTICS.md`](./ANALYTICS.md#known-gaps--accepted-risk).
