...
> **Crontab bug, now fixed:** `docker/crontab` previously had the `analyticsRecoveryWorker.js` line pointing at `cd /path/to/app` (a literal placeholder, not the container's real working directory) and `jobs/analyticsRecoveryWorker.js` (wrong path — the script lives in `src/cron/`), with output redirected to `/var/log/analytics-recovery.log` instead of the `/proc/1/fd/1`/`/proc/1/fd/2` pattern used elsewhere. All three are corrected now: the recovery job runs from `/usr/src/app` like the other cron line, points at the correct script path, and its output flows into the same Vector/Axiom pipeline as everything else. This was silently defeating the crash-recovery fix in production until fixed — worth double-checking after the next deploy that `analyticsRecoveryWorker` log lines are actually showing up.
...
- ~~**The `analyticsWorker`/`flushClicksWorker` cadence (every minute) still leaves a small, non-zero durability exposure window**~~ — **Fixed**, see [Scheduled Jobs](#scheduled-jobs) — the claim-based flush plus `analyticsRecoveryWorker` closes the previously-missing replay path, and the crontab path bug that was silently preventing the recovery worker from running has also been corrected.
