...
export const createShortUrl = tryCatch(async (req, res, next) => {
  ...
  const input = new URL(validated.data.full_url);
  const app = new URL(config.app.baseUrl);

  if (
    input.origin === app.origin &&
    /^\/[A-Za-z0-9_-]+$/.test(input.pathname)
  ) {
    throw new ValidationError({
      full_url: "Cannot shorten a URL generated by this service.",
    });
  }
  if (req.user) {
